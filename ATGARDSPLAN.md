...
- [x] **0.1** Skapa en tag/release i GitHub på nuvarande `main` (namn: `fore-atgardsplan-2026-07`). GitHub web: Releases → Draft a new release → tag `fore-atgardsplan-2026-07` → Publish.
- [x] **0.2** Committa denna fil (`ATGARDSPLAN.md`) till repo-roten.
- [x] **0.3** Öppna appen live och kör ett känt fungerande ärende (t.ex. artikel 1229844) som baslinje. Notera att uppslag + kommentar fungerar. Detta är referensen för all regressionskontroll.
...
- [x] **5.1** `api/gemini.js`: `const { text } = req.body ?? {}` (idag kraschar 500 om body saknas).
- [x] **5.2** `api/gemini.js`: höj `maxOutputTokens` till 2048 (långa ärenden trunkerar JSON → tyst regex-fallback).
- [x] **5.3** `app.js`, `showDHLCard`: kör `esc()` på `shipmentNumber`, `latestStatus`, `latestDate` och `userName` innan de interpoleras i innerHTML/value-attribut.
