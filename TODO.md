...
**PRIO E — LÖST 2026-07-14.** Kunddata skickas inte längre som URL-query till
Vercel. Både Magento- och Puzzel-bokmärket använder nu URL-fragment (`#`)
istället för query (`?`) — fragment skickas per webbstandard aldrig till
servern, så datan når aldrig Vercels loggar.

Den ursprungliga planen (postMessage-handskakning) visade sig onödigt komplex.
Fragment-lösningen ger samma skydd mot serverloggning med en bråkdel av risken.

Implementerat:
- Båda bokmärkena bygger URL med `#` istället för `?`
- `app.js` läser från `location.hash` (query kvar som fallback för äldre bokmärken)
- `history.replaceState` rensar adressfältet direkt efter inläsning
- `hashchange`-lyssnare laddar om sidan när nytt ärende öppnas i redan öppen flik
  (annars uppdaterades inte appen — webbläsaren ser fragmentbyte som "samma sida")

Verifierat live med både Magento- och Puzzel-bokmärket: URL:en är ren, all data
kommer fram. Se `DATAFLOW.md` punkt 5.

**Kvarstående, medveten avvägning:** kunddata i webbläsarens historik (lokal risk
på arbetsdatorn, inte data till tredjepart). Kunddata skickas fortfarande till
DHL:s eget bokningsverktyg — avsiktligt, det är data som ska till DHL.
